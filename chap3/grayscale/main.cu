...
void colortoGrayscaleConvertion_CPU(unsigned char* out, unsigned char* data, int width, int height, int num_channels)
{
...
{   
    // CPU 
    {
        ...
        clock_t start = clock();
        colortoGrayscaleConvertion_CPU(Pout_h, data,x,y,n);
        clock_t end = clock();
    ...
    // GPU
    {
        ...
        const char* output_filename = "./sample_gray_gpu.jpg";
        if (!stbi_write_jpg(output_filename, x, y, 1, Pout_h, 100)) {
            printf("Failed to save the image to %s\n", output_filename);
            return 1;
        }
